...
void ManyBuckets::addItem(unsigned long item) {
	...
	unsigned long placement, range;
	
	if (numBuckets == 1)
		range = ULONGMAX;
	else
		range = (ULONGMAX / numBuckets) + 1;

	placement = item / range;
	arr[placement].push_back(item);
	
}

// TODO:  For this function, loop over every item and call addItem on the pre-made global buckets object
...
void sortEachBucketMultithreaded() {
	...
	//	myThreads[i] = thread(sortEachBucket(), i);
	//}
	//for (int i = 0; i < NUM_THREADS; i++) {
	//	myThreads[i].join();
	//}
...
void combineBuckets() {
	...
	for (int i = 0; i < numBuckets; i++)
	{
		...
		{
			list[count] = globalBuckets->getBucket(i)[i];
			count++;
		}
	}
}
